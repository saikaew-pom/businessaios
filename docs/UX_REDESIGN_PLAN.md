...
**ยังไม่ทำในทุกเฟส**: component library (button/input/modal/toast แบบ reusable), icon set เดียวแทน emoji ทั้งระบบ, Turnstile เปิดใช้งานจริง (ปิดไว้ตั้งใจ — ดู STATUS.md), dark mode ครบทุกหน้า (เฟส E แปลงแค่ Profile, เฟส H เพิ่ม dashboard, เฟส I เพิ่ม login/register, เฟส J เพิ่ม project wizard — เหลืออีก ~33 หน้า รวมทุกหน้า tools/*), primary-600 link contrast gap บนหน้า Profile/Dashboard เก่า (รู้แล้วว่ามี แต่ยังไม่แก้ย้อนหลัง), sticky header bug (P2-4 — หาไม่เจอ อาจไม่มีอยู่แล้ว), 22 จุดใน error message ภายใน (`'AI call failed'` ฯลฯ) ที่เห็นได้เฉพาะใน admin panel เท่านั้น — ทิ้งไว้ตั้งใจ (ไม่ใช่หน้าที่ corporate network filter จะ block)

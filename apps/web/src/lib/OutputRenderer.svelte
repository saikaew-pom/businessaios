  ...
  <div class="space-y-4">
    ...
    {#if output.anti_positioning}
      <div class="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl p-5">
        <div class="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider mb-2">ไม่ใช่ลูกค้าเรา</div>
        <div class="text-sm text-dark-900 dark:text-dark-50 leading-relaxed">{output.anti_positioning}</div>
      </div>
    {/if}

    {#if output.reasoning}
      <details class="bg-dark-50 dark:bg-dark-900 border border-dark-100 dark:border-dark-700 rounded-xl p-5">
        <summary class="cursor-pointer text-sm font-semibold text-dark-900/70 dark:text-dark-100/70">💡 ทำไม ระบบอัจฉริยะ ถึงแนะนำแบบนี้</summary>
        <div class="mt-3 text-sm text-dark-900/80 dark:text-dark-100/80 leading-relaxed">{output.reasoning}</div>
      </details>
    {/if}
  </div>
      ...
      {#each output.personas as persona, i}
        <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-2xl p-6">
          <div class="flex items-start gap-3 mb-4">
            ...
            <div class="flex-1">
              <div class="font-semibold text-dark-900 dark:text-dark-50">{persona.name || `Persona ${i+1}`}</div>
              {#if persona.size_estimate}
                <div class="text-xs text-dark-900/60 dark:text-dark-100/60">≈ {persona.size_estimate}</div>
              {/if}
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-2 gap-4 mb-4">
            {#if persona.demographics}
              <div class="bg-dark-50 dark:bg-dark-900 rounded-lg p-4">
                <div class="text-xs font-semibold text-primary-600 dark:text-primary-400 mb-2">ข้อมูลส่วนตัว</div>
                <dl class="text-sm space-y-1">
                  {#if persona.demographics.age}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">อายุ</dt><dd>{persona.demographics.age}</dd></div>{/if}
                  {#if persona.demographics.job}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">อาชีพ</dt><dd>{persona.demographics.job}</dd></div>{/if}
                  {#if persona.demographics.income}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">รายได้</dt><dd>{persona.demographics.income}</dd></div>{/if}
                  {#if persona.demographics.location}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">ที่อยู่</dt><dd>{persona.demographics.location}</dd></div>{/if}
                </dl>
              </div>
            {/if}
            {#if persona.psychographics}
              <div class="bg-dark-50 dark:bg-dark-900 rounded-lg p-4">
                <div class="text-xs font-semibold text-primary-600 dark:text-primary-400 mb-2">ความสนใจ/ความกลัว</div>
                <dl class="text-sm space-y-1">
                  {#if persona.psychographics.values}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">ค่านิยม</dt><dd>{persona.psychographics.values}</dd></div>{/if}
                  {#if persona.psychographics.fears}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">กลัว</dt><dd>{persona.psychographics.fears}</dd></div>{/if}
                  {#if persona.psychographics.aspirations}<div class="flex"><dt class="w-20 text-dark-900/60 dark:text-dark-100/60">ใฝ่ฝัน</dt><dd>{persona.psychographics.aspirations}</dd></div>{/if}
                </dl>
              </div>
            {/if}
          </div>
          ...
          {#if persona.preferred_channels && persona.preferred_channels.length > 0}
            <div class="mb-3">
              <div class="text-xs font-semibold text-primary-600 dark:text-primary-400 mb-2">📱 ช่องทางที่ใช้</div>
              <div class="flex flex-wrap gap-2">
                {#each persona.preferred_channels as ch}
                  <span class="px-2.5 py-1 bg-primary-50 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 text-xs rounded-full">{ch}</span>
                {/each}
              </div>
            </div>
          {/if}
          ...
          {#if persona.best_offer}
            <div class="bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 rounded-lg p-3 mt-3">
              <div class="text-xs font-semibold text-green-700 dark:text-green-400 mb-1">🎯 ข้อเสนอที่ตรงใจ</div>
              <div class="text-sm text-dark-900 dark:text-dark-50">{persona.best_offer}</div>
            </div>
          {/if}
        </div>
      {/each}
  ...
  <div class="space-y-3">
    {#if output.journey && output.journey.length > 0}
      {#each output.journey as stage, i}
        <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-2xl p-5 relative">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                <h4 class="font-semibold text-dark-900 dark:text-dark-50">{stage.stage_name_th || stage.stage}</h4>
                <span class="text-xs text-dark-900/50 dark:text-dark-100/50 uppercase tracking-wider">{stage.stage}</span>
              </div>

              {#if stage.emotions}
                <div class="mt-2 text-sm text-dark-900/80 dark:text-dark-100/80">
                  ...
                  {#if stage.emotions.quote}
                    <div class="italic text-dark-900/60 dark:text-dark-100/60 mt-1">"{stage.emotions.quote}"</div>
                  {/if}
                </div>
              {/if}
              ...
              {#if stage.touchpoints && stage.touchpoints.length > 0}
                <div class="mt-3 flex flex-wrap gap-1.5">
                  {#each stage.touchpoints as t}
                    <span class="px-2 py-0.5 bg-dark-50 dark:bg-dark-900 text-dark-900/70 dark:text-dark-100/70 text-xs rounded">{t}</span>
                  {/each}
                </div>
              {/if}

              {#if stage.key_message}
                <div class="mt-3 bg-primary-50 dark:bg-primary-900/40 border-l-4 border-primary-500 p-3 text-sm">
                  <span class="font-semibold text-primary-700 dark:text-primary-300">ข้อความหลัก: </span>{stage.key_message}
                </div>
              {/if}
    ...
    {#if output.pain_points && output.pain_points.length > 0}
      <div class="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl p-5 mt-4">
        <div class="text-xs font-semibold text-red-700 dark:text-red-400 uppercase tracking-wider mb-3">⚠️ จุดเจ็บที่ต้องระวัง</div>
        {#each output.pain_points as pp}
          <div class="mb-3 last:mb-0">
            <div class="text-xs font-semibold text-dark-900/70 dark:text-dark-100/70 uppercase">{pp.stage}</div>
            <div class="text-sm text-dark-900 dark:text-dark-50 mb-1"><span class="text-red-600 dark:text-red-400">ปัญหา:</span> {pp.pain}</div>
            <div class="text-sm text-dark-900 dark:text-dark-50"><span class="text-green-600 dark:text-green-400">วิธีแก้:</span> {pp.solution}</div>
          </div>
        {/each}
      </div>
    {/if}
...
{:else if step === 4 && output}
  ...
  <div class="space-y-4">
        ...
        <div class="space-y-2">
          {#each output.tagline_options as tag, i}
            <div class="flex items-center gap-3 p-3 bg-dark-50 dark:bg-dark-900 rounded-lg">
              ...
              <span class="text-sm font-medium italic text-dark-900 dark:text-dark-50">"{tag}"</span>
            </div>
          {/each}
        </div>
      </div>
    ...
    {#if output.competitive_frame}
      <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-xl p-5">
        <div class="text-xs font-semibold text-primary-600 dark:text-primary-400 uppercase tracking-wider mb-3">vs คู่แข่ง</div>
        <div class="space-y-2">
          {#each Object.entries(output.competitive_frame) as [k, v]}
            ...
            <div class="text-sm"><span class="font-semibold text-primary-700 dark:text-primary-300">{label}:</span> {v}</div>
          {/each}
        </div>
      </div>
    {/if}
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {#each output.calendar as post}
          <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-xl p-4 hover:shadow-md transition">
            ...
            <div class="font-semibold text-sm text-dark-900 dark:text-dark-50 mb-2 leading-snug">
              "{post.hook}"
            </div>
            <div class="text-xs text-dark-900/80 dark:text-dark-100/80 leading-relaxed mb-3 whitespace-pre-line">
              {post.caption}
            </div>
            ...
            {#if post.hashtags && post.hashtags.length > 0}
              <div class="flex flex-wrap gap-1 mb-2">
                {#each post.hashtags as h}
                  <span class="text-xs text-dark-900/60 dark:text-dark-100/60">{h}</span>
                {/each}
              </div>
            {/if}
            {#if post.visual_suggestion}
              <div class="text-xs text-dark-900/60 dark:text-dark-100/60 italic pt-2 border-t border-dark-100 dark:border-dark-700">
  ...
  <div class="space-y-4">
    {#if output.workflows && output.workflows.length > 0}
      {#each output.workflows as wf}
        <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-2xl p-5">
          <div class="flex items-start justify-between mb-3">
            <div>
              <h4 class="font-semibold text-dark-900 dark:text-dark-50">{wf.name}</h4>
              <div class="text-xs text-dark-900/60 dark:text-dark-100/60 mt-0.5">ID: {wf.id} · {wf.type}</div>
            </div>
            <div class="text-right">
              <div class="text-2xl font-bold text-green-600 dark:text-green-400">-{wf.time_saved_pct}%</div>
              <div class="text-xs text-dark-900/60 dark:text-dark-100/60">เวลา</div>
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
            <div class="bg-red-50 dark:bg-red-950/40 rounded-lg p-2 text-center">
              <div class="text-xs text-dark-900/60 dark:text-dark-100/60">ก่อนใช้ ระบบอัจฉริยะ</div>
              <div class="font-semibold text-red-700 dark:text-red-400">{wf.time_before}</div>
            </div>
            <div class="bg-green-50 dark:bg-green-950/40 rounded-lg p-2 text-center">
              <div class="text-xs text-dark-900/60 dark:text-dark-100/60">หลังใช้ ระบบอัจฉริยะ</div>
              <div class="font-semibold text-green-700 dark:text-green-400">{wf.time_after}</div>
            </div>
          </div>

          {#if wf.steps && wf.steps.length > 0}
            <div class="space-y-2 mb-3">
              <div class="text-xs font-semibold text-primary-600 dark:text-primary-400 uppercase tracking-wider">ขั้นตอน</div>
              {#each wf.steps as step}
                <div class="flex gap-3 text-sm">
                  <span class="w-6 h-6 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 flex items-center justify-center text-xs font-bold flex-shrink-0">{step.step}</span>
                  <div class="flex-1">
                    <div class="font-medium text-dark-900 dark:text-dark-50">{step.action}</div>
                    <div class="text-xs text-dark-900/60 dark:text-dark-100/60">⏱ {step.duration} · 📦 {step.output}</div>
                  </div>
                </div>
              {/each}
            </div>
          ...
          {#if wf.tools_used && wf.tools_used.length > 0}
            <div class="text-xs text-dark-900/70 dark:text-dark-100/70 mb-2">
              🛠 <span class="font-semibold">เครื่องมือ:</span> {wf.tools_used.join(', ')} (~{wf.tools_cost_monthly} บาท/เดือน)
            </div>
          {/if}
          <div class="text-xs text-dark-900/70 dark:text-dark-100/70">
          ...
          {#if wf.pitfalls && wf.pitfalls.length > 0}
            <details class="mt-3">
              <summary class="text-xs font-semibold text-amber-700 dark:text-amber-400 cursor-pointer">⚠️ ข้อควรระวัง</summary>
              <ul class="mt-2 text-xs space-y-1">
                {#each wf.pitfalls as p}<li class="text-dark-900/80 dark:text-dark-100/80">• {p}</li>{/each}
              </ul>
            </details>
          {/if}
        </div>
    ...
    {#if output.voice_guide}
      <div class="bg-gradient-to-br from-primary-50 to-white dark:from-primary-900/40 dark:to-dark-900 border border-primary-200 dark:border-primary-800 rounded-2xl p-5">
        ...
        <div class="grid md:grid-cols-2 gap-3 text-sm">
          <div>
            <div class="text-xs font-semibold text-green-700 dark:text-green-400 mb-1">✅ ควรทำ</div>
            <ul class="space-y-1">{#each output.voice_guide.do as d}<li>• {d}</li>{/each}</ul>
          </div>
          <div>
            <div class="text-xs font-semibold text-red-700 dark:text-red-400 mb-1">❌ ไม่ควรทำ</div>
        ...
        {#if output.voice_guide.sample_phrases && output.voice_guide.sample_phrases.length > 0}
          <div class="mt-3 pt-3 border-t border-primary-200 dark:border-primary-800">
            <div class="text-xs font-semibold text-dark-900/70 dark:text-dark-100/70 mb-2">ตัวอย่างประโยค</div>
            <div class="space-y-1">
              {#each output.voice_guide.sample_phrases as p}
                <div class="text-sm italic text-dark-900 dark:text-dark-50">"{p}"</div>
              {/each}
            </div>
          </div>
        {/if}
...
{:else if step === 7 && output}
  ...
  <div class="space-y-4">
    {#if output.kpis && output.kpis.length > 0}
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {#each output.kpis as kpi}
          <div class="bg-white dark:bg-dark-800 border border-dark-100 dark:border-dark-700 rounded-xl p-5">
            <div class="text-xs text-dark-900/60 dark:text-dark-100/60 mb-1">{kpi.id}</div>
            <div class="font-semibold text-dark-900 dark:text-dark-50 mb-3">{kpi.name}</div>
            <div class="flex items-end gap-3 mb-3">
              <div>
                <div class="text-2xl font-bold text-dark-900/40 dark:text-dark-100/40">{kpi.current}</div>
                <div class="text-xs text-dark-900/60 dark:text-dark-100/60">ปัจจุบัน</div>
              </div>
              <svg class="w-6 h-6 text-dark-900/30 dark:text-dark-100/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg>
              <div>
                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{kpi.target_30d}</div>
                <div class="text-xs text-dark-900/60 dark:text-dark-100/60">30 วัน</div>
              </div>
              <div>
                <div class="text-2xl font-bold text-primary-600 dark:text-primary-400">{kpi.target_90d}</div>
                <div class="text-xs text-dark-900/60 dark:text-dark-100/60">90 วัน</div>
              </div>
            </div>
            <div class="text-xs text-dark-900/70 dark:text-dark-100/70 space-y-1">
              <div><span class="text-dark-900/50 dark:text-dark-100/50">หน่วย:</span> {kpi.unit}</div>
              <div><span class="text-dark-900/50 dark:text-dark-100/50">เครื่องมือ:</span> {kpi.tool}</div>
              <div><span class="text-dark-900/50 dark:text-dark-100/50">ความถี่:</span> {kpi.frequency}</div>
              <div><span class="text-dark-900/50 dark:text-dark-100/50">คนดูแล:</span> {kpi.owner}</div>
            </div>
            {#if kpi.action_if_below}
              <div class="mt-2 pt-2 border-t border-dark-100 dark:border-dark-700 text-xs text-amber-700 dark:text-amber-400">
    ...
    {#if output.review_ritual}
      <div class="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl p-5">
        <div class="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider mb-3">🔁 Review Ritual</div>
...
{:else if output}
  ...
  <div class="bg-dark-50 dark:bg-dark-900 rounded-lg p-4 text-sm text-dark-900/70 dark:text-dark-100/70">
    Output ยังไม่อยู่ในรูปแบบที่กำหนด — ลอง regenerate
  </div>
  <pre class="mt-3 bg-dark-50 dark:bg-dark-900 border border-dark-100 dark:border-dark-700 rounded-lg p-4 text-xs overflow-auto max-h-96 text-dark-900/80 dark:text-dark-100/80 font-mono">{JSON.stringify(output, null, 2)}</pre>
{/if}

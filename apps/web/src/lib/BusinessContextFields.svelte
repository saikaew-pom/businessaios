...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
      {#each BUSINESS_TYPES as type}
        <button
          ...
          class="text-left p-2.5 rounded-lg border-2 transition {businessType === type.id ? 'border-primary-500 bg-primary-50 shadow-sm' : 'border-dark-200 bg-white hover:border-primary-300 hover:bg-primary-50/30'}"
          title={type.description}
        >
          <div class="text-2xl mb-1">{type.icon}</div>
          <div class="text-xs font-semibold text-dark-900 leading-tight">{type.label}</div>
        </button>
      {/each}
    </div>
    {#if businessTypeError}
      <div class="text-xs text-red-600 mt-1">{businessTypeError}</div>
    {/if}
    {#if businessType}
  ...
  {#if businessType}
    <div>
      ...
      <select
        bind:value={industry}
        class="w-full px-3 py-2.5 rounded-lg border border-dark-200 focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white"
      >
      ...
      {#if industry === '__custom__'}
        <input
          ...
          class="w-full mt-2 px-3 py-2.5 rounded-lg border border-dark-200 focus:outline-none focus:ring-2 focus:ring-primary-500"
        />
      {/if}
      {#if industryError}
        <div class="text-xs text-red-600 mt-1">{industryError}</div>
      {/if}
      {#if !industry}
        <div class="text-xs text-dark-900/50 mt-1">เลือกจาก {availableIndustries.length} ตัวเลือก หรือพิมพ์เอง</div>
      {/if}
    </div>
  {/if}
  ...
  <div>
    <div class="flex items-center justify-between mb-1.5">
      <label class="block text-sm font-semibold">กลุ่มเป้าหมาย <span class="text-dark-900/50 font-normal">(เลือกได้ 1 ต่อหมวด · ไม่บังคับทุกหมวด)</span></label>
      {#if targetAudiences.length > 0}
        <span class="text-xs text-primary-700 font-semibold">เลือก {targetAudiences.length} กลุ่ม</span>
      {/if}
    </div>
      ...
      <div class="mb-3 flex flex-wrap gap-1.5">
        {#each targetAudiences as id}
          <button
            ...
            class="inline-flex items-center gap-1 px-2.5 py-1 bg-primary-100 text-primary-800 text-xs rounded-full hover:bg-primary-200"
          >
            <span>{TARGET_AUDIENCES.find(a => a.id === id)?.icon}</span>
            <span>{getAudienceLabel(id)}</span>
            <span class="text-primary-600 ml-1">✕</span>
          </button>
        {/each}
      </div>
    {/if}

    <div class="bg-dark-50 border border-dark-200 rounded-lg p-3 space-y-3 max-h-72 overflow-y-auto">
      {#each Object.keys(audiencesByCategory) as catKey}
        <div>
          <div class="text-xs font-bold text-dark-900/70 mb-1.5">{AUDIENCE_CATEGORY_LABELS[catKey]}</div>
          <div class="flex flex-wrap gap-1.5">
            {#each audiencesByCategory[catKey] as a}
              ...
              <button
                ...
                class="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-md border transition {isSelected ? 'bg-primary-500 text-white border-primary-500' : 'bg-white text-dark-900 border-dark-200 hover:border-primary-400 hover:bg-primary-50'}"
                title={a.description}
              >
                <span>{a.icon}</span>
                <span>{a.label}</span>
    ...
    <input
      ...
      class="w-full mt-2 px-3 py-2 text-sm rounded-lg border border-dashed border-dark-300 focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white"
    />
  </div>
</div>

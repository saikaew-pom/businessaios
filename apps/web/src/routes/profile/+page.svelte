...
<div class="min-h-screen bg-dark-50">
  ...
  <main class="container-narrow py-10 space-y-6 max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-dark-100 p-6">
      ...
      <div class="flex items-center gap-4 mb-6 pb-6 border-b border-dark-100">
        ...
        <div class="flex-1">
          ...
          <p class="text-xs text-dark-900/50 mt-1">สูงสุด 1MB · จะถูก resize อัตโนมัติ</p>
          {#if avatarMsg}
            <p class="text-xs mt-1 {avatarMsg.startsWith('✓') ? 'text-green-700' : 'text-red-700'}">{avatarMsg}</p>
          {/if}
        </div>
      </div>
      ...
      <form onsubmit={handleSaveProfile} class="space-y-4">
        <div class="grid sm:grid-cols-2 gap-3">
          <div>
            ...
            <input id="first_name" type="text" bind:value={first_name} class="w-full px-3 py-2.5 rounded-lg border border-dark-200 focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          <div>
            <label for="last_name" class="block text-sm font-medium mb-1.5">นามสกุล</label>
            <input id="last_name" type="text" bind:value={last_name} class="w-full px-3 py-2.5 rounded-lg border border-dark-200 focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
        </div>
        <div>
          <label for="email" class="block text-sm font-medium mb-1.5">อีเมล</label>
          <input id="email" type="email" value={$fullUser?.email || ''} disabled class="w-full px-3 py-2.5 rounded-lg border border-dark-200 bg-dark-50 text-dark-900/60" />
          <p class="text-xs text-dark-900/50 mt-1">เปลี่ยนอีเมลไม่ได้ในเวอร์ชันนี้</p>
        </div>
        <div>
          <label for="phone" class="block text-sm font-medium mb-1.5">เบอร์โทร</label>
          <input id="phone" type="tel" bind:value={phone} placeholder="08x-xxx-xxxx" class="w-full px-3 py-2.5 rounded-lg border border-dark-200 focus:outline-none focus:ring-2 focus:ring-primary-500" />
        </div>

        <div class="flex items-center justify-between pt-2">
          <div class="text-sm">
            {#if $fullUser?.email_verified}
              <span class="text-green-700">✓ ยืนยันอีเมลแล้ว</span>
            {:else}
              <button type="button" onclick={() => sendVerification().then(() => alert('ส่งอีเมลยืนยันแล้ว'))} class="text-amber-700 hover:underline font-semibold">
        ...
        {#if saveMsg}
          <div class="text-sm {saveMsg.startsWith('✓') ? 'text-green-700' : 'text-red-700'}">{saveMsg}</div>
        {/if}
      </form>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-dark-100 p-6">
      ...
      <p class="text-sm text-dark-900/60 mb-4">1 credit ≈ $0.001 (≈ 1K input tokens)</p>
      ...
      <details class="text-sm">
        ...
        <div class="mt-3 space-y-2 max-h-64 overflow-y-auto">
          {#each history as tx}
            <div class="flex items-center justify-between p-2 rounded bg-dark-50 text-xs">
              <div>
                <div class="font-semibold {tx.delta > 0 ? 'text-green-700' : 'text-red-700'}">
                  {tx.delta > 0 ? '+' : ''}{tx.delta}
                </div>
                <div class="text-dark-900/60">{tx.reason}</div>
              </div>
              <div class="text-right">
                <div class="text-dark-900/60">→ {tx.balance_after}</div>
                <div class="text-dark-900/40">{formatDate(tx.created_at)}</div>
              </div>
            </div>
          {/each}
        </div>
    ...
    <div class="bg-white rounded-2xl border border-dark-100 p-6">
      ...
      <form onsubmit={handleChangePassword} class="space-y-3 mb-6 pb-6 border-b border-dark-100">
        <h3 class="text-sm font-semibold">เปลี่ยนรหัสผ่าน</h3>
        <input type="password" bind:value={oldPwd} placeholder="รหัสผ่านเดิม" class="w-full px-3 py-2.5 rounded-lg border border-dark-200" />
        <input type="password" bind:value={newPwd} placeholder="รหัสผ่านใหม่ (≥ 8 ตัว)" class="w-full px-3 py-2.5 rounded-lg border border-dark-200" />
        {#if pwdMsg}
          <div class="text-sm {pwdMsg.startsWith('✓') ? 'text-green-700' : 'text-red-700'}">{pwdMsg}</div>
        {/if}
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-dark-900/60 mt-0.5">เมื่อเปิด ระบบจะส่งรหัส OTP ไปทางอีเมลทุกครั้งที่ login</p>
        </div>
        <button

  ...
  <div class="w-full max-w-md">
    ...
    <div class="bg-white dark:bg-dark-800 rounded-2xl shadow-xl border border-dark-100 dark:border-dark-700 p-8">
      {#if step === 'email'}
        ...
        <p class="text-sm text-dark-900/60 dark:text-dark-100/60 mb-6">กรอกอีเมลที่ใช้สมัคร ระบบจะส่งรหัส OTP ไปให้</p>
        <form onsubmit={handleEmail} class="space-y-4">
          <div>
            <label for="email" class="block text-sm font-medium mb-1.5">อีเมล</label>
            <input id="email" type="email" bind:value={email} required class="w-full px-4 py-2.5 rounded-lg border border-dark-200 dark:border-dark-600 focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="you@example.com" />
          </div>
        ...
        <p class="text-sm text-center text-dark-900/60 dark:text-dark-100/60 mt-6">
          <a href="/login" class="text-primary-600 dark:text-primary-400 hover:underline">← กลับไปหน้า Login</a>
        </p>
      {:else if step === 'otp'}
        <h1 class="text-2xl font-bold mb-2">📬 ใส่รหัส OTP</h1>
        <p class="text-sm text-dark-900/60 dark:text-dark-100/60 mb-6">ส่งรหัส OTP ไปที่ <b>{email}</b> แล้ว กรุณาเช็คอีเมล</p>
        <form onsubmit={handleOtp} class="space-y-4">
          <div>
            <label for="otp" class="block text-sm font-medium mb-1.5">รหัส OTP 6 หลัก</label>
            <input id="otp" type="text" bind:value={otp} required maxlength="6" pattern="[0-9]{6}" class="w-full px-4 py-3 text-center text-2xl font-bold tracking-widest rounded-lg border border-dark-200 dark:border-dark-600 focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="000000" />
          </div>
          <div>
            <label for="newPwd" class="block text-sm font-medium mb-1.5">รหัสผ่านใหม่ (≥ 8 ตัว)</label>
            <input id="newPwd" type="password" bind:value={newPwd} required minlength="8" class="w-full px-4 py-2.5 rounded-lg border border-dark-200 dark:border-dark-600 focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          {#if error}<div class="p-3 rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-sm text-red-700 dark:text-red-400">{error}</div>{/if}
          <button type="submit" disabled={isLoading} class="btn-primary w-full disabled:opacity-50">
        ...
        <button onclick={() => step = 'email'} class="block w-full mt-3 text-sm text-dark-900/60 dark:text-dark-100/60 hover:underline">
          ← เปลี่ยนอีเมล
        </button>
      {:else}
        <div class="text-center">
          <div class="text-5xl mb-4">✅</div>
          <h1 class="text-xl font-bold mb-2 text-green-700 dark:text-green-400">เปลี่ยนรหัสผ่านเรียบร้อย</h1>
          <p class="text-sm text-dark-900/60 dark:text-dark-100/60 mb-6">กรุณา login ใหม่ด้วยรหัสผ่านใหม่</p>

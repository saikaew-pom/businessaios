...
<div class="space-y-6">
  ...
  {#if sourceItems.length === 0}
    <div class="text-center py-8 text-dark-900/50">
      ⚠️ ไม่มี source content — กรุณากลับไป Step 4 เพื่อใส่ข้อมูล
    </div>
  {:else}
    <div class="space-y-2 max-h-96 overflow-y-auto border border-dark-200 rounded-lg p-3">
      {#each sourceItems.slice(0, 5) as item, i}
        <div class="text-xs bg-dark-50 p-2 rounded">
          <strong>{item.title || `Item ${i + 1}`}:</strong> {(item.content || '').slice(0, 120)}...
        </div>
      {/each}
      {#if sourceItems.length > 5}
        <div class="text-xs text-dark-900/50 text-center">...และอีก {sourceItems.length - 5} รายการ</div>
      {/if}
    </div>
  {/if}
  ...
  {#if initialData}
    <div class="mt-8 border-t pt-6">
      ...
      {#if initialData.must_have?.length}
        <div class="mb-4">
          <div class="text-sm font-semibold text-green-700 mb-2">✅ Must Have ({initialData.must_have.length})</div>
          <div class="space-y-2">
            {#each initialData.must_have as item}
              <div class="bg-green-50 border border-green-200 rounded-lg p-3 text-sm">
                <div class="font-semibold">{item.title}</div>
                <div class="text-dark-900/70 mt-1">{item.content}</div>
                <div class="text-xs text-green-700 mt-1">→ {item.why_must} (slide {item.slide_target})</div>
              </div>
            {/each}
          </div>
        </div>
      ...
      {#if initialData.maybe?.length}
        <div class="mb-4">
          <div class="text-sm font-semibold text-amber-700 mb-2">⏸️ Maybe ({initialData.maybe.length})</div>
          <div class="space-y-2">
            {#each initialData.maybe as item}
              <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
                <div class="font-semibold">{item.title}</div>
                <div class="text-dark-900/70 mt-1">{item.content}</div>
                <div class="text-xs text-amber-700 mt-1">→ {item.placement}</div>
              </div>
            {/each}
          </div>
        </div>
      ...
      {#if initialData.kill_it?.length}
        <div class="mb-4">
          <div class="text-sm font-semibold text-red-700 mb-2">🗑️ Kill It ({initialData.kill_it.length})</div>
          <div class="space-y-2">
            {#each initialData.kill_it as item}
              <div class="bg-red-50 border border-red-200 rounded-lg p-3 text-sm line-through opacity-70">
                <div class="font-semibold">{item.title}</div>
                <div class="text-dark-900/70 mt-1">{item.content}</div>
                <div class="text-xs text-red-700 mt-1 not-italic">→ {item.reason}</div>
              </div>
            {/each}
          </div>
        </div>

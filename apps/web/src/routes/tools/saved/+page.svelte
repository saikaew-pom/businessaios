...
<div class="min-h-screen bg-dark-50 dark:bg-dark-950">
  <main class="container-narrow py-8">
    ...
    {#if isLoading}
      <div class="text-center py-12 text-dark-900/60 dark:text-dark-100/60">กำลังโหลด...</div>
    ...
    {:else}
      <div class="space-y-3">
        {#each saves as save}
          <div class="bg-white dark:bg-dark-800 rounded-xl border border-dark-100 dark:border-dark-700 hover:border-primary-200 dark:hover:border-primary-700 transition overflow-hidden">
            <div class="p-4 flex items-start gap-3">
              ...
              >
                ...
                <div class="text-xs text-dark-900/50 dark:text-dark-100/50 mt-1 flex items-center gap-2">
                  <span>อัปเดต {formatDate(save.updated_at)}</span>
                  <span class="text-dark-900/30 dark:text-dark-100/30">•</span>
                  <span class="text-primary-600 dark:text-primary-400">{expandedSave === save.id ? '▼ ซ่อน' : '▶ ดูเนื้อหา'}</span>
                </div>
              </button>
            ...
            <!-- Expanded content -->
            {#if expandedSave === save.id}
              <div class="border-t border-dark-100 dark:border-dark-700 bg-dark-50/50 dark:bg-dark-900/50 p-4">
                ...
                <div class="flex items-center gap-2 mb-4 flex-wrap">
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-1 px-3 py-2 rounded-lg bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 text-sm hover:bg-dark-50"
                  >
                    📝 Markdown
                  </button>
                  <button
                ...
                {#if loadingDetails === save.id}
                  <div class="text-center text-sm text-dark-900/60 dark:text-dark-100/60 py-4">กำลังโหลด...</div>
                      ...
                      <div class="space-y-3">
                        ...
                        {#if save.tool_type === 'brand_voice'}
                          ...
                          {#if output.personality_archetype || output.tone}
                            <div class="bg-gradient-to-br from-primary-50 to-blue-50 dark:from-primary-900/40 dark:to-blue-950/40 border border-primary-200 dark:border-primary-800 rounded-lg p-4">
                            ...
                            <div class="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-lg p-4">
                              ...
                              <ul class="space-y-1.5 text-sm text-red-900 dark:text-red-200">
                                {#each output.dont_list as x}
                                  <li class="flex gap-2"><span class="text-red-600 dark:text-red-400 flex-shrink-0">•</span><span>{x}</span></li>
                                {/each}
                              </ul>
                            </div>
                          {/if}
                            ...
                            {#if output.vocabulary.use_words?.length || output.vocabulary.avoid_words?.length}
                              <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                  ...
                                  <div class="mb-3">
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                      {#each output.vocabulary.use_words as w}
                                        <span class="text-xs px-2.5 py-1 rounded-full bg-blue-100 dark:bg-blue-950/40 text-blue-800 dark:text-blue-300 font-medium">{w}</span>
                                      {/each}
                                    </div>
                                  </div>
                                {/if}
                                {#if output.vocabulary.avoid_words?.length}
                                  <div>
                                    <div class="text-xs text-red-700 dark:text-red-400 font-semibold mb-1.5">ห้ามใช้</div>
                                    <div class="flex flex-wrap gap-1.5">
                                      {#each output.vocabulary.avoid_words as w}
                                        <span class="text-xs px-2.5 py-1 rounded-full bg-red-100 dark:bg-red-950/40 text-red-800 dark:text-red-300 font-medium line-through">{w}</span>
                                      {/each}
                                    </div>
                                  </div>
                                {/if}
                              ...
                              <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                ...
                                <div class="space-y-3">
                                  {#each sampleEntries as [ctx, list]}
                                    <div>
                                      <div class="text-xs font-semibold text-primary-700 dark:text-primary-300 mb-1.5">{ctx}</div>
                                      <div class="space-y-1.5">
                                        {#each list as p}
                                          <div class="text-sm italic text-dark-900/80 dark:text-dark-100/80 pl-3 border-l-2 border-primary-200 dark:border-primary-800">"{p}"</div>
                                        {/each}
                                      </div>
                                    </div>
                                  {/each}
                              ...
                              <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                ...
                                <div class="space-y-3">
                                  {#each exEntries as [ctx, content]}
                                    <div>
                                      <div class="text-xs font-semibold text-primary-700 dark:text-primary-300 mb-1.5">{ctx}</div>
                                      <div class="text-sm text-dark-900/80 dark:text-dark-100/80 whitespace-pre-wrap bg-dark-50 dark:bg-dark-950 p-2.5 rounded">{content}</div>
                                    </div>
                                  {/each}
                                </div>
                              </div>
                                ...
                                {#each output.pain_points as pp}
                                  <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                    <div class="flex items-start gap-2 mb-1.5">
                                      <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 text-xs font-bold flex-shrink-0">{pp.rank || '?'}</span>
                                      <div class="font-semibold text-sm text-dark-900 dark:text-dark-50">{pp.title}</div>
                                    </div>
                        ...
                        {:else if save.tool_type === 'persona_builder'}
                          {#if output.disclaimer}
                            <div class="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm text-amber-800 dark:text-amber-300 flex gap-2">
                            ...
                            <div>
                              ...
                              <div class="space-y-3">
                                {#each output.personas as p}
                                  <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                    <div class="font-semibold text-base text-dark-900 dark:text-dark-50">{p.name}</div>
                                    {#if p.tag}<div class="text-xs text-dark-900/60 dark:text-dark-100/60 mb-2">{p.tag}</div>{/if}
                                    {#if p.demographics}
                                      <div class="text-xs text-dark-900/70 dark:text-dark-100/70 mb-2 grid grid-cols-2 gap-x-3">
                        ...
                        {:else if save.tool_type === 'competitor_analysis'}
                                  ...
                                  <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4">
                                    <div class="flex items-start justify-between gap-2 mb-2">
                                      <div class="font-semibold text-sm text-dark-900 dark:text-dark-50">
                                        <span class="text-dark-900/40 dark:text-dark-100/40 mr-1">#{i+1}</span> {c.name}
                                      </div>
                            ...
                            <div>
                              ...
                              <div class="space-y-2">
                                ...
                                {#if output.recommended_strategy.next_30_days?.length}
                                  <div class="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                                    <div class="text-xs font-bold text-blue-800 dark:text-blue-300 uppercase mb-1.5">📅 30 วัน</div>
                                    <ul class="space-y-0.5 text-sm text-blue-900 dark:text-blue-200">{#each output.recommended_strategy.next_30_days as a}<li class="flex gap-1.5"><span>→</span>{a}</li>{/each}</ul>
                                  </div>
                                {/if}
                                {#if output.recommended_strategy.next_90_days?.length}
                                  <div class="bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 rounded-lg p-3">
                                    <div class="text-xs font-bold text-indigo-800 dark:text-indigo-300 uppercase mb-1.5">🎯 90 วัน</div>
                                    <ul class="space-y-0.5 text-sm text-indigo-900 dark:text-indigo-200">{#each output.recommended_strategy.next_90_days as a}<li class="flex gap-1.5"><span>→</span>{a}</li>{/each}</ul>
                                  </div>
                                {/if}
                                {#if output.recommended_strategy.avoid}
                                  <div class="bg-rose-50 dark:bg-rose-950/40 border-l-4 border-rose-400 dark:border-rose-600 rounded-r-lg p-3 text-sm text-rose-900 dark:text-rose-200">
                                    <strong class="text-rose-800 dark:text-rose-300 text-xs uppercase">🚫 ห้ามทำ:</strong> {output.recommended_strategy.avoid}
                                  </div>
                                {/if}
                              </div>
                            </div>
                          {/if}
                          {#if output.reasoning}
                            <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-4 text-sm italic text-dark-900/70 dark:text-dark-100/70">
                              💡 {output.reasoning}
                            </div>
                          {/if}
                            ...
                            <div>
                              ...
                              <div class="space-y-1.5">
                                {#each output.customer_decision_timeline as t}
                                  <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded p-2.5 text-xs">
                                    <div class="font-bold text-primary-700 dark:text-primary-300">💡 {t.stage_name_th || t.stage}</div>
                                    {#if t.customer_thinks}<div class="text-dark-900/80 dark:text-dark-100/80">💭 {t.customer_thinks}</div>{/if}
                                    {#if t.marketing_opportunity}
                                      <div class="bg-primary-50 dark:bg-primary-900/40 border-l-2 border-primary-400 dark:border-primary-600 rounded-r p-1.5 mt-1 text-primary-900 dark:text-primary-200">
                            ...
                            <div>
                              ...
                              <div class="grid sm:grid-cols-2 gap-2">
                                {#each output.triggers as t}
                                  <div class="bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded p-2.5 text-xs">
                                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300">{t.type}</span>
                                    <div class="text-sm font-medium text-dark-900 dark:text-dark-50 mt-1">{t.event}</div>
                                  </div>
                                {/each}
                              </div>
                            </div>
                        ...
                        {:else if save.tool_type === 'business_model_canvas'}
                          ...
                          {#if output.executive_insight}
                            <div class="bg-indigo-50 dark:bg-indigo-950/40 border-l-4 border-indigo-500 dark:border-indigo-600 p-3 rounded text-xs italic text-indigo-900 dark:text-indigo-200">
                                ...
                                {#each (output.customer_segments || []) as s}
                                  <div class="bg-white dark:bg-dark-800 rounded p-2 text-xs">
                                    <div class="flex items-center gap-1.5 mb-0.5">
                                      <span class="px-1.5 py-0.5 rounded text-[10px] border {s.priority === 'primary' ? 'bg-red-100 dark:bg-red-950/40 text-red-800 dark:text-red-300 border-red-300 dark:border-red-700' : s.priority === 'secondary' ? 'bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border-amber-300 dark:border-amber-700' : 'bg-blue-100 dark:bg-blue-950/40 text-blue-800 dark:text-blue-300 border-blue-300 dark:border-blue-700'}">{s.priority}</span>
                                    </div>
                                    <div class="font-semibold text-blue-900 dark:text-blue-200">{s.name}</div>
                                    <div class="text-blue-800 dark:text-blue-300 text-[11px] mt-0.5">{s.description}</div>
                                  </div>
                                {/each}
                              </div>
                            </div>
                              ...
                              <div class="space-y-1.5">
                                {#each (output.value_propositions || []) as v}
                                  <div class="bg-white dark:bg-dark-800 rounded p-2 text-xs">
                                    ...
                                    <div class="text-purple-800 dark:text-purple-300 text-[11px] italic mt-0.5">"{v.vp_statement}"</div>
                                    {#if v.differentiator}<div class="text-purple-700 dark:text-purple-400 text-[10px] mt-0.5">⭐ {v.differentiator}</div>{/if}
                                  </div>
                                {/each}
                              </div>
                            </div>
                              ...
                              <div class="space-y-1.5">
                                {#each (output.customer_relationships || []) as r}
                                  <div class="bg-white dark:bg-dark-800 rounded p-2 text-xs">
                                    ...
                                    <div class="text-cyan-800 dark:text-cyan-300 text-[10px] mt-0.5">[{r.type}] [{r.motivation}] {r.intensity}</div>
                                    {#if r.example}<div class="text-cyan-700 dark:text-cyan-400 text-[10px] italic mt-0.5">"{r.example}"</div>{/if}
                                  </div>
                                {/each}
                              </div>
                            </div>
                              ...
                              <ul class="space-y-0.5 text-xs text-amber-900 dark:text-amber-200">
                                {#each (output.key_resources || []) as r}
                                  <li class="bg-white dark:bg-dark-800 rounded p-1.5 mb-0.5">
                                    <span class="text-[10px] text-amber-700/70 dark:text-amber-400/70">[{r.importance}] [{r.type}]</span><br />
                                    {r.description}
                                  </li>
                                {/each}
                              </ul>
                            ...
                            <div class="bg-orange-50 dark:bg-orange-950/40 border-l-4 border-orange-500 dark:border-orange-600 rounded p-3">
                              ...
                              <ul class="space-y-0.5 text-xs text-orange-900 dark:text-orange-200">
                                {#each (output.key_activities || []) as a}
                                  <li class="bg-white dark:bg-dark-800 rounded p-1.5 mb-0.5">
                                    <span class="text-[10px] text-orange-700/70 dark:text-orange-400/70">[{a.importance}] [{a.type}]</span><br />
                                    {a.description}
                                  </li>
                                {/each}
                              </ul>
                              ...
                              <ul class="space-y-0.5 text-xs text-pink-900 dark:text-pink-200">
                                {#each (output.key_partnerships || []) as p}
                                  <li class="bg-white dark:bg-dark-800 rounded p-1.5 mb-0.5">
                                    ...
                                    <div class="text-[10px] text-pink-700/70 dark:text-pink-400/70">[{p.type}] {p.motivation}</div>
                            ...
                            <div class="bg-emerald-50 dark:bg-emerald-950/40 border-l-4 border-emerald-500 dark:border-emerald-600 rounded p-3">
                              ...
                              <div class="space-y-1.5">
                                {#each (output.revenue_streams || []) as r}
                                  <div class="bg-white dark:bg-dark-800 rounded p-2 text-xs">
                                    <div class="font-semibold text-emerald-900 dark:text-emerald-200">{r.description}</div>
                                    <div class="text-emerald-700 dark:text-emerald-400 text-[10px] mt-0.5">[{r.type}] [{r.pricing_model}] {r.estimated_share}</div>
                                    <div class="text-emerald-800 dark:text-emerald-300 text-[10px]">💵 {r.price_range}</div>
                                  </div>
                                {/each}
                              </div>
                            </div>
                        ...
                        {:else if save.tool_type === 'million_dollar_offer'}
                          ...
                          {#if output.what_makes_it_unbeatable}
                            <div class="bg-amber-50 dark:bg-amber-950/40 border-l-4 border-amber-500 dark:border-amber-600 p-3 rounded text-xs italic text-amber-900 dark:text-amber-200">
                              ...
                              <div class="grid grid-cols-4 gap-1.5 text-xs">
                                {#each [
                                  ...
                                  <div class="bg-white dark:bg-dark-800 rounded p-1.5 text-center">
                                    <div class="text-[10px] text-amber-700 dark:text-amber-400">{l.label} {l.dir}</div>
                                    <div class="text-base font-bold {ve[l.key]?.score >= 7 ? 'text-emerald-600 dark:text-emerald-400' : ve[l.key]?.score >= 5 ? 'text-amber-600' : 'text-red-600 dark:text-red-400'}">{ve[l.key]?.score || '-'}/10</div>
                                  </div>
                                {/each}
                              </div>
                              ...
                              <div class="space-y-1.5">
                                {#each output.value_stack as v}
                                  <div class="rounded p-2 text-xs {v.is_core ? 'bg-amber-100 dark:bg-amber-950/40 border-2 border-amber-400 dark:border-amber-600' : 'bg-white'}">
                                    ...
                                    {#if v.addresses_obstacle}<div class="text-[10px] text-emerald-700 dark:text-emerald-400 mt-0.5">→ {v.addresses_obstacle}</div>{/if}
                                  </div>
                                {/each}
                              </div>
                            </div>
                          ...
                          {#if output.guarantee}
                            ...
                            <div class="bg-blue-50 dark:bg-blue-950/40 border border-blue-300 dark:border-blue-700 rounded p-3 text-sm">
                              ...
                              <div class="bg-white dark:bg-dark-800 rounded p-2 border border-blue-200 dark:border-blue-800">
                                ...
                                <div class="text-xs text-blue-800 dark:text-blue-300 mt-1">{g.terms || '-'}</div>
                                <div class="text-[10px] text-blue-700 dark:text-blue-400 mt-1">⏱️ {g.duration || '-'}</div>
                              </div>
                            </div>
                          {/if}
                          ...
                          {#if output.offer_name}
                            ...
                            <div class="bg-amber-50 dark:bg-amber-950/40 border-2 border-amber-400 dark:border-amber-600 rounded-lg p-3 text-center">
                              ...
                              <div class="grid grid-cols-5 gap-1 mt-2 text-[10px]">
                                ...
                                <div class="bg-white dark:bg-dark-800 rounded p-1"><b>A</b><br/>{n.avatar || '-'}</div>
                                <div class="bg-white dark:bg-dark-800 rounded p-1"><b>G</b><br/>{n.goal || '-'}</div>
                                <div class="bg-white dark:bg-dark-800 rounded p-1"><b>I</b><br/>{n.interval || '-'}</div>
                                <div class="bg-white dark:bg-dark-800 rounded p-1"><b>C</b><br/>{n.container || '-'}</div>
                              </div>
                            </div>
                          {/if}
                              ...
                              {#each output.objections as o, i}
                                <div class="bg-white dark:bg-dark-800 border-2 border-rose-200 dark:border-rose-800 rounded-lg p-3">
                                  <div class="flex items-center gap-1.5 mb-1 flex-wrap">
                                    <span class="text-[10px] text-rose-700 dark:text-rose-400 font-bold">#{i + 1}</span>
                                    <span class="px-1.5 py-0.5 rounded text-[10px] border bg-rose-100 dark:bg-rose-950/40 text-rose-800 dark:text-rose-300 border-rose-300 dark:border-rose-700">{o.category}</span>
                                    <span class="px-1.5 py-0.5 rounded text-[10px] bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border-amber-300 dark:border-amber-700">{o.reframe_strategy}</span>
                                  </div>
                              ...
                              <div class="space-y-1">
                                {#each output.faq_top_5 as f, i}
                                  <div class="border-l-2 border-rose-400 dark:border-rose-600 bg-rose-50 dark:bg-rose-950/40 rounded-r p-1.5">
                                    <div class="text-rose-900 dark:text-rose-200"><b>Q{i + 1}:</b> {f.q}</div>
                                    <div class="text-rose-800 dark:text-rose-300 text-[10px] mt-0.5">→ {f.a}</div>
                                  </div>
                                {/each}
                              </div>
                            </div>
                        ...
                        {:else if save.tool_type === 'hook_library'}
                          ...
                          {#if output.brand_voice_summary}
                            <div class="bg-teal-50 dark:bg-teal-950/40 border-l-4 border-teal-500 dark:border-teal-600 p-3 rounded text-xs italic text-teal-900 dark:text-teal-200">
                          ...
                          {#if output.hook_categories?.length}
                            ...
                            <div class="space-y-2">
                              {#each output.hook_categories as cat}
                                <div class="bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 rounded p-2.5 text-xs">
                                  <div class="font-bold text-teal-900 dark:text-teal-200 mb-1.5">📌 {cat.thai_label || cat.name}</div>
                                  <div class="space-y-1">
                                    {#each (cat.examples || []).slice(0, 3) as ex}
                                      <div class="bg-white dark:bg-dark-800 rounded p-1.5 border-l-2 border-teal-400 dark:border-teal-600">
                                        <div class="text-teal-900 dark:text-teal-200 italic">"{ex.hook}"</div>
                                        <div class="text-[10px] text-teal-700 dark:text-teal-400 mt-0.5">→ {ex.best_for} · CTA: {ex.cta}</div>
                                      </div>
                                    {/each}
                                  </div>
                                </div>
                              ...
                              <div class="grid grid-cols-2 gap-1.5 text-[10px]">
                                {#each Object.entries(output.platform_specific) as [platform, hooks]}
                                  <div class="bg-white dark:bg-dark-800 rounded p-1.5">
                                    <div class="font-bold text-cyan-900 dark:text-cyan-200">{platform}</div>
                                    <ul class="text-cyan-800 dark:text-cyan-300 mt-0.5">{#each (hooks as string[]).slice(0, 2) as h}<li>• {h.slice(0, 60)}...</li>{/each}</ul>
                                  </div>
                                {/each}
                              </div>
                            </div>
                        ...
                        {:else}
                          <pre class="whitespace-pre-wrap break-words font-mono bg-white dark:bg-dark-800 border border-dark-200 dark:border-dark-600 rounded-lg p-3 text-xs">{JSON.stringify(output, null, 2)}</pre>
                        {/if}
                      </div>
                    </div>
                  {/if}
                {:else}
                  <div class="text-center text-sm text-dark-900/60 dark:text-dark-100/60 py-4">ไม่มีข้อมูล</div>
                {/if}
              </div>
            {/if}
          </div>

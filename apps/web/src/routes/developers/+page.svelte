  ...
  <main class="container-narrow py-10 space-y-8">
    ...
    <div class="flex items-start justify-between gap-4 flex-wrap">
      <div>
        ...
        <p class="text-dark-900/60 dark:text-dark-100/60">เชื่อมต่อ Claude Code หรือ Claude Desktop เข้ากับ Business Smart OS — สร้างแผน, รันเครื่องมือ, export ได้โดยตรงจาก Claude โดยใช้เครดิตเดียวกับบัญชีของคุณ</p>
      </div>
    ...
    {#if revealedToken}
      <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onclick={() => revealedToken = null}>
        <div class="bg-white dark:bg-dark-800 rounded-2xl p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto" onclick={(e) => e.stopPropagation()}>
          <h3 class="font-bold text-lg mb-1">Token สร้างแล้ว ✅</h3>
          <p class="text-sm text-amber-600 dark:text-amber-400 font-semibold mb-4">⚠️ จะแสดงครั้งเดียวเท่านั้น — คัดลอกเก็บไว้ตอนนี้ ปิดหน้าต่างนี้แล้วจะดูอีกไม่ได้</p>

          <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Token</label>
            <div class="flex gap-2">
              <input readonly value={revealedToken.token} class="flex-1 px-3 py-2 rounded-lg border border-dark-200 dark:border-dark-600 font-mono text-xs" />
          ...
          <div class="mb-4">
            ...
            <div class="flex gap-2">
              <pre class="flex-1 px-3 py-2 rounded-lg border border-dark-200 dark:border-dark-600 font-mono text-xs overflow-x-auto whitespace-pre-wrap break-all">{cliCommand(revealedToken.token)}</pre>
          ...
          <div class="mb-5">
            ...
            <div class="flex gap-2">
              <pre class="flex-1 px-3 py-2 rounded-lg border border-dark-200 dark:border-dark-600 font-mono text-xs overflow-x-auto">{jsonConfig(revealedToken.token)}</pre>
    ...
    <div class="bg-white dark:bg-dark-800 rounded-2xl border border-dark-100 dark:border-dark-700 p-5">
      ...
      <div class="flex gap-2">
        <input
          ...
          class="flex-1 px-3 py-2.5 rounded-lg border border-dark-200 dark:border-dark-600 text-sm"
        />
    ...
    <div class="bg-white dark:bg-dark-800 rounded-2xl border border-dark-100 dark:border-dark-700 overflow-hidden">
      <div class="p-4 border-b border-dark-100 dark:border-dark-700 font-semibold text-sm">Token ของคุณ ({tokens.length})</div>
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          {:else if tokens.length === 0}
            <tr><td colspan="6" class="p-6 text-center text-dark-900/50 dark:text-dark-100/50">ยังไม่มี token — สร้างอันแรกด้านบน</td></tr>
          {:else}
            {#each tokens as t}
              <tr class="border-t border-dark-100 dark:border-dark-700">
                ...
                <td class="p-3">
                  {#if t.is_active}
                    <span class="text-xs px-2 py-0.5 rounded-full bg-green-50 dark:bg-green-950/40 text-green-700 dark:text-green-400">ใช้งานอยู่</span>
                  {:else}
                    <span class="text-xs px-2 py-0.5 rounded-full bg-dark-50 dark:bg-dark-950 text-dark-900/50 dark:text-dark-100/50">เพิกถอนแล้ว</span>
                  {/if}
                </td>
                <td class="p-3 text-xs text-dark-900/60 dark:text-dark-100/60">{formatDate(t.created_at)}</td>
                <td class="p-3 text-xs text-dark-900/60 dark:text-dark-100/60">{formatDate(t.last_used_at)}</td>
                <td class="p-3">
                  {#if t.is_active}
                    <button onclick={() => handleRevoke(t)} class="text-xs text-red-600 dark:text-red-400 hover:underline">เพิกถอน</button>
                  {/if}
                </td>
              </tr>
            {/each}
    ...
    <div class="bg-white dark:bg-dark-800 rounded-2xl border border-dark-100 dark:border-dark-700 p-5">
      ...
      <ul class="text-sm text-dark-900/70 dark:text-dark-100/70 space-y-1.5">
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">list_projects</code> — ดูรายการแผนทั้งหมด</li>
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">create_project</code> — สร้างแผนใหม่</li>
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">generate_step</code> — สร้างเนื้อหาแต่ละขั้นตอนด้วยระบบอัจฉริยะ (ใช้เครดิต)</li>
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">run_tool</code> — รันเครื่องมือเดี่ยวทั้ง 10 ตัว (ใช้เครดิต)</li>
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">get_export</code> — export แผนเป็นไฟล์</li>
        <li><code class="text-xs bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">check_credits</code> — เช็คเครดิตคงเหลือ</li>
      </ul>
      <p class="text-xs text-dark-900/50 dark:text-dark-100/50 mt-3">MCP server URL: <code class="bg-dark-50 dark:bg-dark-950 px-1.5 py-0.5 rounded">{mcpUrl}</code></p>
    </div>
  </main>
</div>
